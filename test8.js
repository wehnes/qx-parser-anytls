...
function fixAnytlsLine(line) {
  ...
  if (/,\s*over-tls\s*=\s*true/i.test(line)) {
    line = line.replace(
      ...
      ", over-tls=true, tls-host=" + tlsHost
    );
  } else {
    line += ", over-tls=true, tls-host=" + tlsHost;
  }
...
function forceFixFinalAnytls(line) {
  line = trim(line);
  ...
  if (!/(^|,\s*)tls-verification\s*=/i.test(line)) {
    line += ", tls-verification=false";
  }

  if (!/(^|,\s*)tls-alpn\s*=/i.test(line)) {
    line += ", tls-alpn=" + DEFAULT_ALPN_HEX;
  }

  if (!/(^|,\s*)tls-no-session-ticket\s*=/i.test(line)) {
    line += ", tls-no-session-ticket=true";
  }

  if (!/(^|,\s*)udp-relay\s*=/i.test(line)) {
    line += ", udp-relay=true";
  }
...
var finalResult = result.map(function(line) {
  return forceFixFinalAnytls(line);
});
